    ...
    public void scanDevice() {
        ...
        boolean b = bleManager.scanDevice(new ListScanCallback(5000) {
            ...
            public void onScanning(final ScanResult result) {
                ...
                //LogUtils.LogE("name:"+ mDevice.getName()+"---address:"+ mDevice.getAddress());
                if(mDevice.getName()!= null && mDevice.getName().equals("WQM3H")){
                    LogUtils.LogD("find the device of WQM3H");
                    BluetoothService.this.result = result;
                    EventBus.getDefault().post(new DeviceEvent(mDevice.getName(),mDevice.getAddress()));
                }
            }
            ...
            public void onScanComplete(final ScanResult[] results) {
                runOnMainThread(new Runnable() {
                    @Override
                    public void run() {
                        LogUtils.LogV("onScanComplete");
                        //连接设备
                        EventBus.getDefault().post(new ConnectEvent(result));
                    }
                });
            }
        });
...
//                        }
            ...
            public void onConnectSuccess(BluetoothGatt gatt, int status) {
                LogUtils.LogV("onConnectSuccess");
            ...
            public void onServicesDiscovered(final BluetoothGatt gatt, int status) {
                ...
                runOnMainThread(new Runnable() {
                    @Override
                    public void run() {
                        LogUtils.LogV("onServicesDiscovered");
                        EventBus.getDefault().post(new NotifyEvent(gatt,"ff12","ff02"));
                    }
                });
            }
            ...
            public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
               LogUtils.LogV("onCharacteristicChanged------"+HexUtil.bytesToHexString(characteristic.getValue()));
            ...
            public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
                LogUtils.LogV("onCharacteristicWrite");
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    LogUtils.LogV("写入成功" +HexUtil.bytesToHexString(characteristic.getValue()));
                }
            ...
            public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    LogUtils.LogV("读取成功:" +String.valueOf(HexUtil.encodeHex(characteristic.getValue())));
                }
    ...
    BluetoothGattCallback mBluetoothGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                LogUtils.LogE("onConnectionStateChange=======");
            }
        }
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                LogUtils.LogE("onServicesDiscovered");
            }else{
                LogUtils.LogE("服务发现失败，错误码为:" + status);
            }
        ...
        public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                LogUtils.LogE("读取成功" +String.valueOf(HexUtil.encodeHex(characteristic.getValue())));
            }
        }
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                LogUtils.LogE("写入成功" +String.valueOf(HexUtil.encodeHex(characteristic.getValue())));
            }
        }

        @Override
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            LogUtils.LogE("数据回调" +String.valueOf(HexUtil.encodeHex(characteristic.getValue())));
        }
    };
    ...
    public boolean writedevice(String uuid_service, String uuid_write, byte[] data, BleCharacterCallback callback) {
        return bleManager.writeDevice(uuid_service, uuid_write, data, callback);
    }
